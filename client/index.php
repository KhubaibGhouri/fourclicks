 ...
 <form action="#" method="POST" style="padding-bottom: 20px";>
  <div class="col-sm-3">
    ...
    <option value="">Select State first</option>
			...
			<div class="col-sm-3">
                            <div class="form-group">
                       <label for="sel1">Category / Service</label>
			...
			$sql = $conn->query($result);
			echo '<option value="fav">Favourites Only</option>';
    ...
    ?>
	    ...
	    <button type="submit" name="submit" class="btn btn-info btn-abc">Select</button>
	</form>
	</div>
	</div>
...
<div class="container constrct_margin_adj" style="max-width: 100%;">
  <h3 class="text-center"  style="margin-top: 20px;">Contact Details</h3>
  <table class="table abc">
    <tbody>
    <tr>
    
      <td rowspan="2" style=" text-align: center;"><input type="checkbox" name="as" id="abc" class="chek" value="<?= $rows['u_id'] ;?>" <?php if($que->num_rows > 0){echo 'checked';}?> >Mark as Favourite</td>
    <th>Company Name</th>
				...
				if ($sql->num_rows > 0) {
					...
					?>
	                 ...
	                 <div>
	                 	...
	                 	<?php
	                 	$result = "SELECT ID,BusinessName,Startdate,Enddate,File,TrainingName, EmployeeName FROM  `coder` where Documenttype='Public Laibility Insurance' and user_id='$name' and Enddate > '$date' and FIND_IN_SET('$client_id', file_status);";
...
<?php
			 $result = "SELECT * FROM coder WHERE Documenttype='Indemnity Insurance' and user_id='$name' and Enddate > '$date' and FIND_IN_SET('$client_id', file_status)";
...
<button role="button" class="btn btn-default buttontype" data-toggle="collapse" href="#demo820-<?php echo $row['ID'];?>">Indemnity Insurance (<?=$sql->num_rows; ?>) </button><br>
		 ...
		 $result = "SELECT * FROM coder WHERE user_id='$name' AND  Documenttype='Worker Comp insurance' and Enddate > '$date' and FIND_IN_SET('$client_id', file_status)";
						...
						<?php
		$result = "SELECT * FROM coder WHERE Documenttype='Risk Assesment' and user_id='$name' and Enddate > '$date'  and FIND_IN_SET('$client_id', file_status)";
...
<button role="button" class="btn btn-default buttontype" data-toggle="collapse" href="#demo800-<?php echo $row['ID'];?>">General Risk Assessment (<?=$sql->num_rows; ?>)</button><br>
						...
						<?php
	 $result = "SELECT * FROM coder WHERE Documenttype='Fire Risk Assessments' and user_id='$name' and Enddate > '$date' and FIND_IN_SET('$client_id', file_status)";
...
<button role="button" class="btn btn-default buttontype" data-toggle="collapse" href="#demo1800-<?php echo $row['ID'];?>">Fire Risk Assessment (<?=$sql->num_rows; ?>)</button><br>
						...
						<?php
		$result = "SELECT * FROM coder WHERE Documenttype='Chemical Risk Assessments' and user_id='$name' and Enddate > '$date' and FIND_IN_SET('$client_id', file_status)";
...
<button role="button" class="btn btn-default buttontype" data-toggle="collapse" href="#demo9800-<?php echo $row['ID'];?>">Chemical  Risk Assessment (<?=$sql->num_rows; ?>)</button><br>
						...
						<?php
	$result = "SELECT * FROM coder WHERE Documenttype='Safe Work Method Statement' and user_id='$name' and Enddate > '$date' and FIND_IN_SET('$client_id', file_status)";
	...
	$result = "SELECT ID,BusinessName,Startdate,Enddate,File,TrainingName,EmployeeName FROM  `coder` where Enddate >'$date' and TrainingName != '' and user_id='$name'  and FIND_IN_SET('$client_id', file_status) GROUP BY EmployeeName";
...
$EmployeeName = $row['EmployeeName'];
  $result2 = "SELECT ID,BusinessName,Startdate,Enddate,File,TrainingName, EmployeeName FROM  `coder` where  user_id='$name'and Enddate>'$date' and TrainingName != '' and  EmployeeName = '$EmployeeName'  and FIND_IN_SET('$client_id', file_status) ";

                $sql2 = $conn->query($result2);
	                	?>         
	                   <a class="btn btn-default btn-sm document" role="button" data-toggle="collapse" href="#collapseExample412-<?php echo $row['ID']; ?>" aria-expanded="false" aria-controls="collapseExample"><?php echo $row['EmployeeName']; ?>  (<?=$sql2->num_rows; ?>)</a><br>
...
echo "</table>"; }?>
	    <?php  }}?>
               ...
               <div class="col-md-4">
	 		<h2 class="Lib1">Equipment / Record</h2>
                        <?php   
      if (isset($_POST['names'])) {
				$name = $_POST['names'];
$date = date("Y-m-d");  
				...
				$result = "SELECT ID,BusinessName,Startdate,Enddate,File,SerialNumber,Equipmentname FROM  `coder` where SerialNumber!= ''
				 and user_id='$name'  and `Equipmentname` != '' and  Enddate > '$date' and FIND_IN_SET('$client_id', file_status) GROUP BY Equipmentname";
   ...
   $result2 = "SELECT ID,BusinessName,Startdate,Enddate,File,SerialNumber,Equipmentname,doc_status FROM  `coder` where user_id='$name' and  SerialNumber!= '' and  Equipmentname = '$Equipmentname' and Enddate > '$date' and FIND_IN_SET('$client_id', file_status)";
 $sql2 = $conn->query($result2);
	                ?>         
	    ...
	    <?php  }}?>
	 	...
	 	<div class="col-md-4">
			...
			if (isset($_POST['names'])) {
				...
				$result = "SELECT * FROM coder WHERE user_id='$name' and Enddate < '$date' and Documenttype != '' and FIND_IN_SET('$client_id', file_status)";
				...
				if ($sql->num_rows > 0) {
					while ($row = $sql->fetch_assoc()) {{?>
	                 ...
	                 <div>
	                 	...
	                 	<?php
	                 	$date = date("Y-m-d");
						$result = "SELECT * FROM coder WHERE user_id='$name' AND  Documenttype='Public Laibility Insurance' AND Enddate <  '$date' and FIND_IN_SET('$client_id', file_status)";
...
$date = date("Y-m-d");
						...
						<?php
	$result = "SELECT * FROM coder WHERE user_id='$name' AND  Documenttype='Risk Assesment' AND Enddate <  '$date'  and FIND_IN_SET('$client_id', file_status)";
...
<button role="button" class="btn btn-default buttontype" data-toggle="collapse" href="#demo6800-<?php echo $row['ID'];?>">Chemical  Risk Assessment (<?=$sql->num_rows; ?>)</button><br>
						...
						<?php
	$result = "SELECT * FROM coder WHERE user_id='$name' AND Documenttype='Safe Work Method Statement' AND Enddate <  '$date' and FIND_IN_SET('$client_id', file_status)";
    ...
    // $result = "SELECT ID,BusinessName,Startdate,Enddate,File,TrainingName, EmployeeName, COUNT( * ) FROM  `coder` GROUP BY EmployeeName";
      $result = "SELECT ID,BusinessName,Startdate,Enddate,File,TrainingName, EmployeeName FROM  `coder` where  Enddate <  '$date' and TrainingName != '' and user_id = '$name'  and FIND_IN_SET('$client_id', file_status)  GROUP BY EmployeeName";
				$sql = $conn->query($result);
	                ...
	                while($row = $sql->fetch_assoc()) {
$EmployeeName = $row['EmployeeName'];
    $result2 = "SELECT ID,BusinessName,Startdate,Enddate,File,TrainingName, EmployeeName FROM  `coder` where user_id='$name' and  Enddate <  '$date' and TrainingName != '' and user_id = '$name' and  EmployeeName = '$EmployeeName' and FIND_IN_SET('$client_id', file_status) ";
